...
import { ConnectGmail } from '@/app/components/agent-specific/welcome-agent/connect-gmail'
...
const presetDirectives = [
  {
    ...
    content:
      "Share industry insights about their competitors' challenges and invite them to a call to show what we've done for companies similar to theirs.",
  },
  {
    value: 'simple-custom-welcome',
    label: 'Simple customized welcome email',
    content:
      'Welcome them personalized insights about their business fit and benefits. End with an invitation to an onboarding call.',
  },
  {
    value: 'success-story',
    label: 'Share a success story',
    content:
      'Share a relevant success story from their industry and invite them to discuss achieving similar results.',
  },
  {
    value: 'book-call',
    label: 'Suggest a quick call',
    content:
      'Highlight how we help similar companies and invite them to book a 15-minute intro call.',
  },
  {
    value: 'product-demo',
    label: 'Invite to product demo',
    content:
      'Highlight relevant features for their role and invite them to schedule a personalized demo this week.',
  },
  {
    value: 'custom-upsell',
    label: 'Upsell with a custom loom video',
    content:
      "Highlight ROI-focused features and ask if they'd like a Loom video to see the potential impact it could create for their business.",
  },
  {
    value: 'super-short',
    label: 'Super short email',
    content:
      'Generate a super-short, personalized email (1-2 sentences and only 1 paragraph). End with this exact call to action: &apos;Want me to send over some more info?&apos;.',
  },
  {
    value: 'fully-custom',
    label: 'Custom',
...
export default function WelcomeAgentNew() {
  ...
  const [emailDetails, setEmailDetails] = useState({
    ...
    body: 'Your personalized email will appear here after generation.',
  })
    ...
    const loadAgent = async () => {
      ...
      try {
        ...
        if (agent.configuration?.settings) {
          setSettings({
            sendOnlyWhenConfident:
              agent.configuration.settings.sendOnlyWhenConfident || false,
            reviewBeforeSending:
  ...
  useEffect(() => {
    const loadConnectedAccounts = async () => {
      ...
      try {
        ...
        const connections = await gmailUtils.getWorkspaceConnections(
          workspace.id,
        )
        ...
        if (selectedEmailAccount) {
          const accountExists = connections.some(
            (conn) => conn.email === selectedEmailAccount,
          )
          if (!accountExists) {
  ...
  const handlePresetDirectiveChange = (value: string) => {
    ...
    const selectedPreset = presetDirectives.find(
  ...
  const handleTest = () => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      toast.error('Validation Error', {
        description:
          'Please fill in all required fields before generating email',
      })
      return
    }
  ...
  const handleCopyEmail = () => {
    if (notificationEmail) {
      ...
      toast.success('Email Copied', {
        description:
          'The notification email has been copied to your clipboard.',
      })
    }
  }
  ...
  const handleSave = async (action: 'published' | 'draft') => {
    ...
    try {
      ...
      if (editId) {
        ...
        // Create new agent
        await welcomeAgentUtils.createWelcomeAgent(
          workspace.id,
          cleanedAgentData,
        )
      }
      ...
      toast.success('Success', {
        description: `Welcome agent ${
          action === 'published' ? 'published' : 'saved as draft'
        }`,
      })
      ...
      toast.error('Failed to save welcome agent', {
        description:
          'An error occurred while trying to save the welcome agent.',
      })
    }
  }
  ...
  const handleTestEmail = async (connectionId: string) => {
    ...
    try {
      await gmailUtils.testEmailConnection({
        connectionId,
        workspaceId: workspace.id,
      })
      toast.success('Success', {
  ...
  useEffect(() => {
    const setupNotificationEmail = async () => {
      ...
      try {
        ...
        let email = await mailgunUtils.getNotificationEmail(
          editId,
          workspace.id,
        )
        ...
        if (!email) {
          email = await mailgunUtils.generateNotificationEmail(
            editId,
            workspace.id,
          )
        }
        ...
        setNotificationEmail(
          mailgunUtils.getFullEmailAddress(email.emailLocalPart),
        )
      } catch (error) {
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-900 flex items-center justify-center p-4">
      ...
      <div
        className="w-full max-w-7xl bg-white rounded-xl overflow-hidden flex flex-col relative"
            ...
            <div className={cn('relative', showOnboarding && 'z-40')}>
              ...
              >
                <div className="space-y-4">
                  <div>
                    <Label
                      htmlFor="directive-select"
                      className="text-sm font-medium"
                    >
                      Choose Email Purpose
                    </Label>
                    <Select
                      onValueChange={handlePresetDirectiveChange}
                  ...
                  <div>
                    <BusinessContext
                      ...
                      onWebsiteChange={(value) =>
                        setBusinessInfo((prev) => ({ ...prev, website: value }))
                      }
                      onPurposeChange={(value) => {
                        setBusinessInfo((prev) => ({ ...prev, purpose: value }))
                      }}
                      onAdditionalContextChange={(value) =>
                        setBusinessInfo((prev) => ({ ...prev, context: value }))
                      }
      ...
      {/* Signup Info Dialog */}
      <Dialog
        open={isSignupInfoDialogOpen}
        onOpenChange={setIsSignupInfoDialogOpen}
      >
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Try testing with real signup data!</DialogTitle>
            <DialogDescription>
              Enter actual user information to generate a personalized email
              preview. Don&apos;t worry, no emails will be sent during this
              test.
            </DialogDescription>
          </DialogHeader>
              ...
              >
                <div className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    Connect your email account to send welcome emails
                    automatically.
                  </p>
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleRemoveGmailConnection(account.id!)
                                }
              ...
              >
                <div className="space-y-4">
                  <p className="text-sm text-gray-500">
                    We&apos;ll provide you with a unique email address. Add this
                    to your existing signup form as a notification email.
                    Whenever someone signs up, we&apos;ll receive their details
                    and automatically create a personalized welcome email.
                  </p>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          handleSettingChange('sendOnlyWhenConfident')
                        }
                      />
                    </div>
                    <div className="flex items-center justify-between">
                      <Label htmlFor="review-emails">
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={() =>
                          handleSettingChange('reviewBeforeSending')
                        }
                      />
                    </div>
                  </div>

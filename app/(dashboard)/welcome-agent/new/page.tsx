...
import { type GmailTokens, gmailUtils } from "@/firebase/gmail-utils";
...
export default function WelcomeAgentNew() {
  ...
  const [generationStep, setGenerationStep] = useState<EmailGenerationStep>("user-info");
  ...
  }, [loading, workspace?.id, selectedEmailAccount]);
  ...
  const handleSave = async (action: "published" | "draft") => {
    ...
    try {
      ...
      toast.error("Failed to save welcome agent", {
        description: "An error occurred while trying to save the welcome agent.",
      });
    }
  };
  ...
  const handleGmailConnected = async (email: string, name: string, tokens: GmailTokens) => {
    ...
    try {
      // Check if user has remaining connected gmail accounts
      if (user?.remainingConnectedGmailAccounts <= 0) {
        toast.error("Error", {
          description: "You have reached the maximum number of connected Gmail accounts",
        });
        return;
      }

      await gmailUtils.saveGmailConnection(workspace.id, user.uid, email, name, tokens);

      // Update user remaining connected gmail accounts
      if (user) {
        setUser({
          ...user,
          remainingConnectedGmailAccounts: user.remainingConnectedGmailAccounts - 1,
        });
      }

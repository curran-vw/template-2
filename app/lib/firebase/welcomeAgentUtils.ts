...
export const welcomeAgentUtils = {
  ...
  async generateWelcomeEmail(agent: WelcomeAgent, signupInfo: any) {
    ...
    try {
      ...
      const gmailConnection = await gmailUtils.getConnectionByEmail(
        agent.configuration?.emailAccount || '',
      )
      const senderName =
        gmailConnection?.name || agent.configuration?.emailAccount || 'the team'
        ...
        fetch('https://openrouter.ai/api/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              For extra context, they signed up for: ${
          ...
          return (
            data?.choices?.[0]?.message?.content || 'No user information found.'
          )
        }),
      ...
      const emailBodyResponse = await fetch(
        ...
        if (!res.ok)
          throw new Error(`OpenRouter API error: ${JSON.stringify(data)}`)
        return (
          data?.choices?.[0]?.message?.content || 'Error generating email body.'
        )
      })
      ...
      const senderGmailConnection = await gmailUtils.getConnectionByEmail(
        agent.configuration?.emailAccount || '',
      )
      if (!senderGmailConnection) {
        throw new Error(
          `No Gmail connection found for email: ${agent.configuration?.emailAccount}`,
        )
      }
      ...
      try {
        ...
        await logsUtils.addLog({
          ...
          details: `Generated email for ${
            signupInfo.email || signupInfo.rawContent.email
          }`,
          workspaceId: agent.workspaceId,
          agentId: agent.id,
        })

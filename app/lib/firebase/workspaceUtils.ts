...
export async function createWorkspace(
  ...
  try {
    ...
    if (!existingSnapshots.empty) {
      console.log(
        'Workspace with this name already exists, returning existing one',
      )
...
export async function updateWorkspaceMembers(
  ...
  try {
    ...
    if (action === 'remove' && workspaceData.ownerId !== userId) {
      console.error(
        'Access denied: Only the workspace owner can remove members',
      )
      return false
    }
    ...
    await updateDoc(workspaceRef, {
      members:
        action === 'add' ? arrayUnion(targetUserId) : arrayRemove(targetUserId),
    })
    return true
  } catch (error) {
    console.error(
      `Error ${action === 'add' ? 'adding' : 'removing'} workspace member:`,
      error,
    )
    return false
  }
...
export async function updateWorkspaceName(
  ...
  try {
    ...
    if (workspaceData.ownerId !== userId) {
      console.error(
        'Access denied: Only the workspace owner can update the name',
      )
      return false
    }

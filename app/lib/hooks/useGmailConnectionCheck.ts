...
import { gmailUtils } from '../firebase/gmailUtils'
...
export function useGmailConnectionCheck(workspaceId: string, intervalMinutes = 60) {
  ...
  useEffect(() => {
    ...
    const startCheck = () => {
      ...
      gmailUtils.checkAndFixInactiveConnections(workspaceId)
        .then(result => {
          lastCheckTimeRef.current = new Date()
          lastCheckResultRef.current = result
          console.log('Gmail connection check completed:', result)
        })
        .catch(error => {
  ...
  const checkNow = () => {
    ...
    isCheckingRef.current = true
    console.log('Manually starting Gmail connection check')
    
    return gmailUtils.checkAndFixInactiveConnections(workspaceId)
      .then(result => {
        lastCheckTimeRef.current = new Date()
        lastCheckResultRef.current = result
        console.log('Manual Gmail connection check completed:', result)
        return result
      })
      .catch(error => {
